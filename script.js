// ================================
// ExchangeRate Dashboard
// ================================

const apiKey = "f28e54cd1a7b7a09e2874900";
const EXCHANGE_API = "https://v6.exchangerate-api.com/v6";

// Frankfurter = free, keyless, ECB-based historical data.
// ExchangeRate-API's free plan does not include a historical endpoint
// (that's Pro/Business only), so 1M / 1Y / 5Y charts pull from Frankfurter
// while the live converter + table keep using ExchangeRate-API as requested.
const FRANKFURTER_API = "https://api.frankfurter.dev/v1";

// how long a "latest rates" fetch stays valid before we hit the API again
const CACHE_TTL_MS = 5 * 60 * 1000;

// how often the 1D tracker records a live point (ms)
const POLL_INTERVAL_MS = 10 * 60 * 1000;

let chart;
let latestCache = null; // { base, rates, fetchedAt }
let historyCache = {};  // key: `${from}_${to}_${range}` -> { labels, data }
let changeCache = {};   // key: `${from}_${to}_${dateISO}` -> previous rate
...
// ================================
// Shared: fetch latest rates (cached)
// ================================
...
// ================================
// Currency Converter
// (only re-fetches when "From" changes; amount / "To" reuse the cache)
// ================================
...
// cached per from/to/date so amount keystrokes don't trigger new requests
...
// ======================
// 1D: live points via localStorage
// ======================
...
function recordLivePoint(from, to, rate) {
    ...
    // throttle: only record a new point if a minute has passed since the last one
    // (prevents typing in the Amount field from spamming duplicate points)
    ...
    // if this pair is the one currently on screen, refresh the chart
...
async function getFrankfurterSeries(from, to, range) {
    ...
    // historical data never changes, so once fetched we reuse it for the session
    ...
    // downsample longer ranges client-side so the chart stays readable
...
// ======================
// Convert triggers
// (amount just recomputes from cache inside convertCurrency, no wasted calls)
// ======================
